...
from transformer.models_rd import Raindrop_v2
...
def prepare_dataloader(opt):
    ...
    print(f""" 
        ...
        percentage: {N_sub}
    """)
    train_data = train_data[:int(N_sub/100 * len(train_data) )]
...
def train_epoch(model, training_data, optimizer, pred_loss_func, opt):
    ...
    for batch in tqdm(training_data, mininterval=2,
        ...
        ## reg loss
        reg_loss = torch.mean(prediction[2]**2)
        
        # loss = event_loss + pred_loss + se / scale_time_loss + reg_loss
    ...
    return total_event_ll / total_num_event, total_event_rate / total_num_pred, rmse, total_reg_loss
...
def train(model, training_data, validation_data, optimizer, scheduler, pred_loss_func, opt):
    ...
    for epoch_i in range(opt.epoch):
        ...
        start = time.time()
        train_event, train_type, train_time, reg_loss = train_epoch(model, training_data, optimizer, pred_loss_func, opt)
        print('  - (Training)    loglikelihood: {ll: 8.5f}, '
              'accuracy: {type: 8.5f}, RMSE: {rmse: 8.5f}, '
              'elapse: {elapse:3.3f} min, '
               .format(ll=train_event, type=train_type, rmse=train_time, elapse=(time.time() - start) / 60))
        print(f"reg: {reg_loss:.2f}")

        start = time.time()
        valid_event, valid_type, valid_time, reg_loss = eval_epoch(model, validation_data, pred_loss_func, opt)
...
def main():
    ...
    # global_structure = torch.eye(d_inp)

    # d_ob = 4
    # opt.d_model2 = d_inp * d_ob
    # opt.max_len = 100
    # # **********************************************************

    # model = Raindrop_v2(
    #     d_inp=opt.C,
    #     d_model=opt.d_model2,
    #     nhead=opt.n_head,
    ...
    #     # perc=0.5,
    #     aggreg='mean',
    #     # n_classes=2, # not important. for downstream classification
